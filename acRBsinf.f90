...
!	impurity separation
 ...
 IF(assm.NE.0 .AND. assm.NE.1) then
 	PRINT *, "acRBsinf.f90: 'assm' should be 0 or 1. Check the inputcard parametros.f90. Stopping!"
 	STOP
 END IF

	do nn=1,2*(P+1+assm)
		if ((assm.EQ.0).AND.(nn.eq.(P+1+assm))) then
			go to 30
		end if
		arg  = nn*pi/(2*(P+1+assm)-assm)
		IF((assm.EQ.1).AND.cos(arg).LE.1.d-14) then
			PRINT *, "acRBsinf.f90: Problem in the assimetric mode. Argument x of 1/cos(x) went zero. Stopping! "
			STOP 
		END IF
		cosn = cos(arg)
...
30	continue
	...
	if(ds.eq.0)then
	   green = zi*(cw-eps00)*green/(4.d0*(P+1+assm)*t**2)
	else
	   green = zi*green/(4.d0*(P+1+assm)*t)
	end if

	denom2	= ((cw-eps00)**2 - t**2)*(P+1+assm)
	sinn1	= sin(pi*(l1-j1)/2.d0)
	sinn2	= sin(pi*(l2-j2)/2.d0)
	if((assm.EQ.0).AND.(dl+dj.eq.0).and.(sl+sj.ne.0)) then
		if(ds.eq.0) then
